...
構成の要点(ポーリング型のスコアリングに対して)
+ 特徴：Scorer を、Kafka を直読する常駐ストリーミングジョブ(Spark Structured Streaming)に。
+ 共通：入口の複数クライアント 〜 CDC 〜 DWH 〜 バッチ学習 〜 BI 可視化はそのまま。
...
04_clickhouse … Kafka Engine が dwh.transactions へ取込(生データ)
  ...
  │  ───────────────────────────────────────
  ▼
07_stream_scorer … Kafka を直読してリアルタイム推論。06 の Model を使う(mtime 監視)
  │  from_json → 特徴量 → 推論(foreachBatch)→ dwh.predictions に書込
  ▼
08_grafana … :3000 で可視化(transactions / predictions)
```

04 → 07 → 08が常時のリアルタイム推論

05→06 で Model を作り、07 がそれを使う


ポイント:**07_stream_scorer は 04 の出力ではなく Kafka トピックを直読**する(04 の Kafka Engineとは別コンシューマグループ)。ポーリングではなくストリームで流れるため、ほぼリアルタイムに`predictions` が埋まる。これが 002 のポーリング型との違い。
...
| | ポーリング型 | 本サンプル(ストリーミング型) |
|---|---|---|
| 推論の起点 | ClickHouse を10秒ごとにポーリング | **Kafka を直接 readStream** |
| 実装 | Python 常駐ループ | **Spark Structured Streaming**(`07_stream_scorer`) |
...
- **Spark の JDBC / ClickHouse 認証エラー(バッチ前処理)** → `clickhouse-jdbc` は `0.7.2-all` を
  使用、`04_clickhouse/users.d/open-default-network.xml` で `default` ユーザの接続元を開放済み。
