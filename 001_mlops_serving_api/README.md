...
```
00_client … 入口クライアント。取引イベントを投入(client_seed.py)
  │  POST /transactions
  ▼
01_api_ingest … Ingest API(FastAPI :8000)。受けた取引を OLTP へ INSERT
  │
  ▼
02_postgres … PostgreSQL / OLTP(wal_level=logical)。行の変更を WAL に出す
  │  WAL(logical decoding)
  ▼
03_debezium … Debezium / Kafka Connect(:8083)。WAL を flat 化し Kafka トピックへ publish
  │  topic: oltp.public.transactions
  │  ※ Kafka(:9092)自体は基盤サービスで、番号フォルダは持たない
  ▼
04_clickhouse … Kafka Engine(Sink:transactions_queue)→ MV → ReplacingMergeTree(DWH / :8123)
  │  JDBC read
  ▼
05_spark … 前処理。重複排除・特徴量生成 → parquet(/data/features)
  │
  ▼
06_trainer … 学習。sklearn Pipeline(OneHot + RandomForest)→ Model(/models/model.joblib)
  │
  ▼
07_api_serving … Serving API(FastAPI :8001)。Model をロードして予測を返す
  │  ▲  POST /predict
  ▼  │  fraud_probability
08_client … 出口クライアント。Serving API に予測を要求(client_predict.py)
```
