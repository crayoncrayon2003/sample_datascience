"""道路網エージェント・フィード(入口 / 実時間ライブ).

osmnx の道路グラフ(GraphML)を読み、車両エージェントが交差点で次の道をランダムに選んで
**実時間**で走行する。各エージェントの緯度経度・速度・走行中の道路リンク(link_id)・道路種別・
制限速度・**実時刻**の hour/dow を Ingest API に送る。

このフィードが **唯一のデータ源**で、地図表示・リアルタイム推論・学習のすべてに使われる。
速度は共有の渋滞モデル congestion.py で決める。偽時間や合成データは無し=実時刻のライブのみ。
